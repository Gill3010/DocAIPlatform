import type { DashboardConversionType, PdfToolItem } from '../constants/conversions';
...
/** Filtra herramientas PDF por query; misma normalización que conversiones. Sin duplicados. */
export function filterPdfToolsByQuery(
    tools: PdfToolItem[],
    query: string
): PdfToolItem[] {
    const trimmed = query.trim();
    if (!trimmed) return tools;
    const tokens = normalizeForSearch(trimmed).split(/\s+/).filter(Boolean);
    if (tokens.length === 0) return tools;
    return tools.filter((tool) => {
        const words = normalizeForSearch(tool.name)
            .split(/\s+/)
            .filter(Boolean);
        return tokens.every((token) =>
            words.some((word) => word.startsWith(token))
