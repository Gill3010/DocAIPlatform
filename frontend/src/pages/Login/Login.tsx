...
import { setPendingAnonymousSessionId, getPendingAnonymousSessionId, removePendingAnonymousSessionId, setToken as setStorageToken } from '../../services/storageService';
...
export const Login = () => {
    ...
    const [isLogin, setIsLogin] = useState(
        (location.state as { mode?: string })?.mode !== 'register'
    );
    ...
    // Persist anon_session_id while user completes login/OAuth redirects
    useEffect(() => {
        const anonFromState = (location.state as { anonymousSessionId?: string } | null)?.anonymousSessionId;
        if (!anonFromState) return;
        setPendingAnonymousSessionId(anonFromState);
    }, [location.state]);
    ...
    const handleGoogleLogin = async () => {
        ...
        try {
            const anon = (location.state as { anonymousSessionId?: string } | null)?.anonymousSessionId ?? getStoredSessionId();
            if (anon) {
                try {
                    setPendingAnonymousSessionId(anon);
                } catch {
                    // ignore
                }
            }
    ...
    const handleFacebookLogin = async () => {
        ...
        try {
            const anon = (location.state as { anonymousSessionId?: string } | null)?.anonymousSessionId ?? getStoredSessionId();
            if (anon) setPendingAnonymousSessionId(anon);
    ...
    useEffect(() => {
        if (facebookAvailable === null) {
            apiService
                .getFacebookAuthUrl()
                .then(() => setFacebookAvailable(true))
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            if (isLogin) {
                ...
                const sessionId =
                    (location.state as { anonymousSessionId?: string } | null)?.anonymousSessionId ??
            ...
            <div className="login-card">
                <div className={`login-header ${isLogin ? 'login-header--login' : 'login-header--register'}`}>
                    <div className="login-header-icon">
                        {isLogin ? (
                            <LogIn size={40} strokeWidth={2} aria-hidden />
                        ) : (
                            <UserPlus size={40} strokeWidth={2} aria-hidden />
                        )}
                    </div>
                ...
                {(googleAvailable || facebookAvailable) && (
                    <p className="login-separator">o continúa con email</p>
                )}
                ...
                <form onSubmit={handleSubmit} className="login-form" noValidate>
                    <div className="form-group">
                        ...
                        <input
                            ...
                            placeholder={isLogin ? 'tu@email.com' : 'tu@email.com'}
                            required
